...
function makeDedupeEquator(options, comparator) {
  var equator = options.dedupe;
  if (typeof equator !== 'function') {
    return function (x, y) {
  ...
  if (options.values) {
    return function (x, y) {
      return equator(x.key, y.key);
    };
  }
  return equator;
}

// Our stream is ordered so we only need to check the previous data to detect
// duplicates
function dedupe(equator) {
  var prev;
  return through2.obj(function (data, enc, callback) {
    if (!prev || !equator(prev, data)) {
      this.push(data);
    }
    prev = data;
    callback();
...
function createMergedReadStream(db, options) {
  ...
  var dedupeEquator = makeDedupeEquator(options, comparator);
  ...
  if (options.ranges) {
    ...
    if (options.dedupe) {
      stream = stream.pipe(dedupe(dedupeEquator));
    }
