...
function makeDudupeEquator(options, comparator) {
...
// detect duplicates
function makeDedupe(equals) {
  var prev;
  return through2.obj(function (data, enc, callback) {
    if (!prev || !equals(prev, data)) {
      this.push(data);
    }
    prev = data;
    callback();
...
function createMergedReadStream(db, options) {
  ...
  var dedupeEquator = makeDudupeEquator(options, comparator);
  ...
  if (options.ranges) {
    ...
    if (options.dedupe) {
      stream = stream.pipe(makeDedupe(dedupeEquator));
    }
